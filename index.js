...
async function fetch_cf_clearance (url) {
	return new Promise((resolve) => {
		cloudscraper.get({
			...
			callback: (a, b, c) => {
				fetch_url.credential.cookies = cookie.parse(b.request.headers.cookie);
				b.request.response.headers['set-cookie'].forEach(c => {
					const ctmp = cookie.parse(c);
					Object.keys(ctmp).map(function (key) {
						if (key !== 'expires' && key !== 'Max-Age' && key !== 'path'  && key !== 'domain') {
							fetch_url.credential.cookies[key] = ctmp[key]
						}
						return null;
					});
				});
...
function fetch_url (action, query, parsing, auth, callback) {
	const cookieObject = fetch_url.credential.cookies;
	const cookies = Object.keys(cookieObject).map(function (key) {
		return cookie.serialize(key, cookieObject[key]);
	});
	const options = {
	...
	request.post(options, function optionalCallback (err, res, buffer) {
		console.log(`${options.method} - ${res.statusCode} - ${options.url}`);
